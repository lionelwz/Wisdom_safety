...
public class FolderAdapter extends BaseAdapter {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        if (holder != null) {
            if(i == 0){
                if(mIsVideo) {
                    holder.name.setText(R.string.mis_folder_all_video);
                    holder.size.setText(String.format("%d%s",
                            getTotalImageSize(), mContext.getResources().getString(R.string.mis_video_unit)));
                } else {
                ...
                if(mFolders.size()>0){
                    ...
                    if (f != null) {
                        ...
                        if(mIsVideo) {
                            Glide.with(mContext)
                                    .load(Uri.fromFile(new File(f.cover.path)))
    ...
    class ViewHolder{
        ...
        void bindData(Folder data) {
            ...
            if (data.cover != null) {
                ...
                if(mIsVideo) {
                    Glide.with(mContext)
                            .load(Uri.fromFile(new File(data.cover.path)))
                            .error(R.drawable.mis_default_error)
                            .override(targetWidth, targetHeight)
                            .centerCrop()
                            .into(cover);
